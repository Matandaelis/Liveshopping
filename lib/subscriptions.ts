...
export async function hasFeatureAccess(
  userId: string,
  feature: 'documents' | 'aiSuggestions' | 'plagiarismScans' | 'collaboration',
): Promise<boolean> {
  try {
    const subscription = await getUserSubscription(userId)
    if (!subscription) return feature === 'documents' || feature === 'collaboration' ? false : true
    ...
    switch (feature) {
      ...
      case 'aiSuggestions': {
        const stats = await prisma.aIUsageStats.findUnique({ where: { userId } })
        return features.maxAISuggestions === -1 || (stats?.suggestionsUsedToday || 0) < features.maxAISuggestions
      }
...
export async function createUserSubscription(userId: string, tier: string = 'FREE') {
  try {
...
export async function initializeUserStats(userId: string) {
  try {
    return await prisma.aIUsageStats.upsert({
      where: { userId },
      update: {},
      create: { userId },
    })
...
export async function updateUserStats(userId: string, data: { words?: number; documents?: number }) {
  try {
    return await prisma.userAnalytics.upsert({
      where: { userId },
      update: {
        totalWordsWritten: data.words ? { increment: data.words } : undefined,
        totalDocumentsCreated: data.documents ? { increment: 1 } : undefined,
      },
      create: {
        userId,
        totalWordsWritten: data.words || 0,
        totalDocumentsCreated: data.documents ? 1 : 0,
      },
    })
  } catch (error) {
